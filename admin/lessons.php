  ...
  while($row = $result->fetch_assoc()){
    if(isset($_REQUEST['checkid']) && $_REQUEST['checkid'] == $row['course_id']){
      ...
      if(($row['course_id']) == $_REQUEST['checkid']){ 
        ...
        ?>
        <h3 class="mt-5 text-white p-2 text-center" style=" background-color:#225470;" >Course ID : <?php if(isset($row['course_id'])) {echo $row['course_id']; } ?> Course Name: <?php if(isset($row['course_name'])) {echo $row['course_name']; } ?></h3>
        <?php
...
<script>
 ...
 <?php if(isset($_SESSION['course_id'])){
   echo '<div><a class="btn btn-danger box" href="./addLesson.php"><i class="fas fa-plus fa-1x"></i>   Add </a></div>';
   } ?>
